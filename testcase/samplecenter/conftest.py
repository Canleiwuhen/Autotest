...
from data_generate.samplecenter.data_route import ROUTE
...
getconf = GetConfig(configname="samplecenter_config.yaml")
...
def login(user):
    ...
    """
    res = Requests(configname="samplecenter_config.yaml", baseurl="test_url")
    # 获取验证码
    time_13 = int(time.time() * 1000)
    uuid = str(time_13) + "_50100645373690044307253736"
    param = {"uuid": uuid}
    content = res.get_request("/ybzx/vcode.do", params=param).content
    cur_path = get_project_path()
    path_tmp = [cur_path, "img/sample_center_code.png"]
    ...
    # 执行接口请求
    login_res = res.post_request("/ybzx/user/login.do", data=data)
...
def token():
    def _token(user):
        ...
        token_json_path = sep([token_json_dir, "sample_center_"+user + "_token.json"])
...
@pytest.fixture
def res():
    res = Requests(configname="samplecenter_config.yaml", baseurl="test_url", headers={"content-type": "application/x-www-form-urlencoded"})
    return res

@pytest.fixture
def send_file():
    send_file = Requests(configname="samplecenter_config.yaml", baseurl="test_url", headers={"multipart/form-data; boundary=----WebKitFormBoundary7TPD6gp0EuUeDAqr"})
    return send_file

@pytest.fixture
def res_json():
    res_json = Requests(configname="samplecenter_config.yaml", baseurl="test_url", headers={"content-type": "application/json;charset=UTF-8"})
    return res_json
...
def generate_steps(request):
    ...
    for n in range(run_times):
        ...
        varlist = vars(tmp)
        # 剔除不需要的类变量
        remove_key = ['token', 'datafactory_res', 'samplecenter_res','samplecenter_upload_res']
